...
public class StrFormatter {
    ...
    public static String format(final String strPattern, final Object... argArray) {
        ...
        for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
            delimIndex = strPattern.indexOf(EMPTY_JSON, handlerPosition);
            if (delimIndex == -1) {
                if (handlerPosition == 0) {
                    return strPattern;
                } else {
                    //字符串模板剩余部分不再包含占位符，加入剩余部分后返回结果
                    sb.append(strPattern, handlerPosition, strPatternLength);
                    return sb.toString();
                }
            } else {
                if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == C_BACKSLASH) {
                    if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == C_BACKSLASH) {
                        ...
                        sb.append(strPattern, handlerPosition, delimIndex - 1);
                    ...
                    // 正常占位符
                    sb.append(strPattern, handlerPosition, delimIndex);
                    sb.append(Convert.utf8Str(argArray[argIndex]));
                    handlerPosition = delimIndex + 2;
                }
            }
        ...
        // 加入最后一个占位符后所有的字符
        sb.append(strPattern, handlerPosition, strPattern.length());
        return sb.toString();
    }
